...
def post_quiz(page, question):
    ...
    # Step 1a: Click the "What's on your mind?" placeholder to expand tab bar
    expand_result = page.evaluate("""(function() {
        var ph = document.querySelector('[placeholder*="mind" i], [aria-label*="mind" i]');
        if (!ph) ph = document.querySelector('#contenteditable-root');
        if (!ph) ph = document.querySelector('[contenteditable][placeholder]');
        if (ph) { ph.click(); ph.focus(); return 'clicked: ' + ph.tagName + '[' + (ph.getAttribute('placeholder')||ph.getAttribute('aria-label')||'') + ']'; }
        return 'composer not found';
    })()""")
    print(f"[expand-composer] {expand_result}")
    time.sleep(3)

    # Debug: dump all elements with direct text node "Quiz"
    quiz_elements = page.evaluate("""(function() {
        var found = [];
        ...
        for (var i = 0; i < all.length; i++) {
            ...
            for (var j = 0; j < el.childNodes.length; j++) {
                if (el.childNodes[j].nodeType === 3) direct += el.childNodes[j].textContent;
            }
            if (/^quiz$/i.test(direct.trim())) {
                found.push(el.tagName + '#' + (el.id||'') + '.' + (el.className||'').substring(0,20) + '[role=' + (el.getAttribute('role')||'') + ']');
            }
        }
    ...
    clicked_quiz = page.evaluate("""(function() {
        ...
        for (var i = 0; i < all.length; i++) {
            ...
            if (/^quiz$/i.test(direct.trim())) {
                var target = el.closest('[role="tab"], button, [tabindex]') || el;
                target.click();
                return 'broad-click: ' + el.tagName + '#' + (el.id||'');
            }
        }
        return null;
    })()""")
